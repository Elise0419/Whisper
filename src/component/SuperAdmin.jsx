    ...
    <div className="adminbody">
      <div className="adminContainer">
          ...
          <thead >
            <tr className="adminTh">
              ...
              <th className="superTime">
                ...
                <button
                  className="sortBtn"
                  onClick={() => toggleSorting("updated_at")}
                >
              ...
              <th>
                ...
                <button
                  className="sortBtn"
                  onClick={() => toggleSorting("login_time")}
                >
              ...
              <th className="superTime">
                ...
                <button
                  className="sortBtn"
                  onClick={() => toggleSorting("logout_time")}
                >
                  {sortField === "logout_time"
                    ? sortOrder === "asc"
                      ? "▲"
                      : "▼"
                    : "無排序"}
            ...
            {data.map((item, index) => (
             ...
             <td className="superTextcon">{item.email}</td>
                ...
                <td>
                  {item.admin.length > 0 ? (
                    <button
                      className="sortdelterBtn"
                      onClick={() => DeleteClick(`${item.user_id}`)}
                    >
                      移除管理員
                    </button>
                  ) : (
                    <button className="sortBtn" onClick={promotion}>
                      提升管理員
                    </button>
                  )}
                </td>
                <td>
                  {item.admin.length > 0 ? (
                    <select
                      ...
                      name=""
                      id=""
                      defaultValue={item.admin[0].type}
                      onChange={(event) => SelectChange(event, item.user_id)}
                    >
        ...
        <button className="sortBtn">
          <Link
            className="adminlink"
            to={`/admin/users/manage/${parseInt(match.params.page) - 1}`}
          >
            上一頁
          </Link>
        </button>
        {Array.from({ length: lastpage }).map((_, index) => (
          <span key={index} className="sortdelterBtn">
            ...
            <Link
              className="adminlink"
              to={`/admin/users/manage/${parseInt(index) + 1}`}
            >
              {index + 1}
            </Link>
            &nbsp;
